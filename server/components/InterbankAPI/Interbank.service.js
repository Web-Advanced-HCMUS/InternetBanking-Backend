...
export async function verifyInterbankDeposit(req, next) {
  try {
    ...
    if (!validSignature) throw new APIError(200, 'Signature is invalid');

    const validBank = await InterbankModel.findOne({
      code: req.body.data.bankCode
    });
    if (!validBank) throw new APIError(200, 'Bank does not be linked in system before');
    ...
    if (!validAccount) throw new APIError(200, 'Beneficiary does not exist in system');
...
export function verifySignature(data, signature, publicKey) {
  try {
    const hashData = md5(JSON.stringify(data));
    ...
    throw new APIError(401, 'Invalid public key format');
  }
}
